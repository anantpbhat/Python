...
def main():
    ...
    for vm in filtered_vms:
        if vm.snapshot is not None:
            ...
            if snapshot is None:
                task = vm.CreateSnapshot_Task(name=snap_name,description=None,memory=mem_switch, quiesce=False)
                print("Snapshot Completed")
                module.exit_json(changed=True)
            else:
                module.exit_json(changed=False,msg="The Snapshot already exists")
        else:
            task = vm.CreateSnapshot_Task(name=snap_name,description=None,memory=mem_switch, quiesce=False)
            print("Snapshot Completed")
            module.exit_json(changed=True)
